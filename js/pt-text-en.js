...
window.PT_TEXT_EN = {
  ...
  prezentare: {
    ...
    intrareCols: ['Parameter', 'Value'],
    intrareRows: [
      ['Installed power (PV modules)',   '{kwp} kWp'],
      ['Installed power (inverters)',    '{pinvKw} kW'],
      ['Operating voltage',              'Un = {unV} V'],
      ['Operating frequency',            'f = 50 Hz'],
      ['Power factor',                   'cos φ > 0.99'],
    ],
    pSuprafata: 'The PV plant will occupy an area of approximately {suprafata} m², with the panels facing {orientare}.',
    pBransament: 'The utility connection (service branch) required by the objective is not part of this documentation.',
    ...
    pRacord1: 'The PV plant connection board (TE-CEF) will be connected at {tensiune} kV in {tablou}. The connection is made in {matACNume} cable, sized from the voltage-drop and thermal-loading conditions, with a length of {lenAC} m.',
    racordCols: ['Circuit', 'AC power [kW]', 'AC current [A]', 'Section [mm²]', 'Breaker [A]', 'ΔU [%]'],
    pRacord2: 'Each inverter is connected on its own circuit, protected by an automatic circuit breaker; the circuits converge on a common residual-current device, per the protections chapter. The sections and rated currents above follow from the calculation report (AC cable sizing relations).',
    pRacordNoinj: 'The installation will be fitted with an active-power monitoring and limiting system (voltage monitoring relay and energy analyser) which supplies the inverters with grid parameter information and regulates the output power according to instantaneous consumption, so that the plant does not inject energy into the distribution operator\'s network.',
    pRacordPlansa: 'The proposed connection solution is detailed in drawing IE002 - Single-line connection diagram.',
    matCu: 'copper (Cu)',
    matAl: 'aluminium (Al)',
    ...
    sInvertor: 'INVERTER TECHNICAL CHARACTERISTICS',
    pInvertor: 'The inverters take the DC energy from the PV modules, across the voltage and current variation ranges, and deliver AC voltage at the output, complying with the technical conditions imposed by the distribution operator. The technical characteristics of the specified inverters are:',
    invCols: ['Parameter', 'Unit'],
    invValCol: 'Value',
    invRows: [
      ['Rated AC power',                          'kW',  'pac'],
      ['Maximum DC input voltage',                'V',   'vinvmax'],
      ['Number of MPP trackers per inverter',     'No.', 'nmppt'],
      ['MPPT voltage range',                      'V',   'vmppt'],
      ['Maximum input current per MPPT',          'A',   'impptmax'],
      ['Maximum short-circuit current per MPPT',  'A',   'iscmppt'],
    ],
  ...
  protectii: {
    title: 'PROTECTION AND SWITCHING EQUIPMENT',
    intro: 'Switching and protection equipment was selected and verified separately on the DC and the AC side, in accordance with I7/2011 and SR EN IEC 62548, respectively with the string fuse sizing relation of the design methodology.',
    dcTitle: 'DC side',
    dcRule: 'The string fuse (gPV) is selected within the window: max(Isc ; 1.25·Imp) <= Inf <= min(2·Isc ; Iprod,PV ; Iprod,inv), where Iprod,PV is the maximum series fuse rating from the module datasheet and Iprod,inv the one from the inverter datasheet. The effective upper bound is the smallest of the available values.',
    dcCols: ['String', 'Isc [A]', '1.25·Imp [A]', 'Inf,min [A]', 'Inf,max [A]', 'gPV selected [A]', 'Required', 'Check'],
    dcReq: 'yes', dcOpt: 'optional',
    dcNoteUn: 'Rated voltage of the fuses and of the DC load-break switch: Un >= {vmax} V (the inverter maximum DC input voltage).',
    dcNoteSpd: 'DC surge protective device, type 2: Uc = {ucdc} V (>= the inverter maximum DC voltage), In >= 10 kA (8/20 µs), Up as low as possible. One device is provided per string.',
    dcNoteSep: 'A manual load-break switch-disconnector is provided at the inverter DC input, with Un >= {vmax} V.',
    dcSingle: 'For a single string on an MPPT input reverse fault current cannot occur, so the string fuse is not mandatory; it becomes necessary with two or more strings connected in parallel. The DC surge arrester remains recommended in all cases.',
    acTitle: 'AC side',
    acCols: ['Circuit', 'I inv,ac [A]', 'Breaker In [A]', 'Check'],
    acRule: 'The breaker of each circuit is selected with In >= I inv,ac, the inverter rated current computed at cos phi = 1.',
    acNoteRcd: 'Residual-current protection: type A RCD; a type B RCD is provided if the inverter does not ensure DC residual current monitoring.',
    acNoteSpd: 'AC surge protective device, type 2, with Uc >= 1.1·Un and In >= max(10 kA ; Isc at the connection point); mandatory where the distance to the metering and protection block exceeds 10 m.',
    acNoteIcc: 'Breaking capacity of the AC breakers: Icn >= the short-circuit current at the connection point = {icc} kA. Thermal and electrodynamic withstand of the equipment is verified in addition.',
    ok: 'verified', bad: 'NOT verified',
  },

  economic: {
    title: 'ECONOMIC ANALYSIS',
    intro: 'The economic analysis starts from the total investment cost and the annual benefit obtained from the energy produced: self-consumed energy is valued at the supply price e1 and the surplus injected into the grid at price e2. The designed system and the "optimum" reference system (modules facing south at the optimal tilt) are compared over an analysis horizon of {n} years, at a discount rate of {rate} %.',
    cols: ['Indicator', 'Unit', 'Designed system', 'Optimum system'],
    rows: {
      prod:  'Annual energy yield',
      eauto: 'Self-consumed energy',
      einj:  'Energy injected into the grid',
      scr:   'Self-consumption rate',
      b:     'Annual benefit (B)',
      inv:   'Investment value (C_PV)',
      tr:    'Payback period (Tr)',
      rir:   'Internal rate of return (IRR)',
      vna:   'Net present value (NPV)',
    },
    umKwh: 'kWh/yr', umPct: '%', umAni: 'years',
    verdictOk: 'Conclusion: the internal rate of return of the designed system ({rir} %) exceeds the discount rate considered ({rate} %), and the net present value is positive ({vna}). The investment is economically efficient over the {n}-year analysis horizon.',
    verdictBad: 'Conclusion: at the parameters considered, the internal rate of return of the designed system ({rir} %) does not exceed the discount rate ({rate} %). Economic efficiency depends on energy price evolution and on any available grant funding.',
  ...
  anexa1: {
    ...
    tAnual:  'Annual',
    lossTitle: 'Conductor power losses',
    lossIntro: 'Conductor power losses are computed per segment, with the relations dP = 2*R*I^2 on the DC and single-phase side and dP = 3*R*I^2 on the three-phase side, where R = rho*l/S. The values are summed and referred to the nominal powers.',
    lossCols: ['Segment', 'L [m]', 'S [mm2]', 'R [ohm]', 'I [A]', 'dP [W]'],
    lossTot: 'Total conductor losses: DC {dcw} W ({dcpct} % of the installed DC power), AC {acw} W ({acpct} % of the AC power), total {totw} W.',
    lossNote: 'The losses above are strictly the conductor losses. The global system efficiency follows from combining them with the inverter conversion efficiency, per its datasheet.',
    brevTitle: 'Calculation report - string sizing',
    brevIntro: 'The number of modules connected in series is chosen so that the string open-circuit voltage, at the minimum design cell temperature, does not exceed the inverter maximum input voltage, while the maximum-power-point voltage at the maximum cell temperature stays within the MPPT operating window. Cell temperature is derived from air temperature and NMOT, applying the ±3% NMOT tolerance. The calculation is presented below for each string, using the catalogue data of the selected module and inverter.',
    brevCols: ['Quantity', 'Relation', 'Numerical substitution', 'Result'],
    brevLbl: {
      tcmin:  'Cell temperature, minimum',
      tcmax:  'Cell temperature, maximum',
      vocmax: 'Module open-circuit voltage, maximum (at Tc,min)',
      vmpmin: 'Module MPP voltage, minimum (at Tc,max)',
      iscmax: 'Module short-circuit current, maximum',
      impmax: 'Module MPP current, maximum',
      vocstr: 'String open-circuit voltage',
    },
    brevNs: 'This gives Ns,min = {nsmin} and Ns,max = {nsmax} modules in series; the adopted configuration uses Ns = {ns} modules in series and Np = {np} string(s) in parallel (max permissible Np = {npmax}).',
    brevOk:  'Check: Voc,string = {vocstr} V <= inverter Umax,dc = {vinvmax} V; Imp,max = {impmax} A <= Imax,MPPT = {impptmax} A; Isc,max = {iscmax} A <= Isc,MPPT = {iscmppt} A. The configuration complies with the inverter limits.',
    brevBad: 'WARNING: the adopted configuration does NOT fully comply with the inverter limits (Voc,string = {vocstr} V against Umax,dc = {vinvmax} V; Imp,max = {impmax} A against {impptmax} A; Isc,max = {iscmax} A against {iscmppt} A). The number of modules in series/parallel must be revised.',
  ...
  planse: {
    ...
    viewSide: 'Side section - to scale',
    viewPlan: 'Top view - to scale',
    noPlan: 'The top view needs the mounting area dimensions (width and depth) from the Mounting step.',
  },
};
